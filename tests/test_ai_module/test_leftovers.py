...
from src.mdf_cutting.ai_module.features.leftovers import (
    Leftover, LeftoverFeatureExtractor
)
from src.mdf_cutting.ai_module.core.leftover_optimizer import (
    LeftoverOptimizationModel, LeftoverOptimizer, LeftoverAssignment
)
...
class TestLeftoverFeatureExtractor:
    ...
    def test_calculate_material_efficiency(self):
        ...
        assert efficiency["total_leftover_area_available"] == 80000.0  # 50000 + 30000
...
class TestLeftoverOptimizationModel:
    ...
    def test_forward_pass(self):
        ...
        assert outputs["strategy"].shape == (batch_size, 10)  # num_leftover_types
    ...
    def test_strategy_probabilities(self):
        ...
        assert torch.allclose(torch.sum(strategy, dim=1), torch.ones(batch_size))
...
class TestLeftoverOptimizer:
    ...
    def test_extract_leftovers_features(self):
        ...
        features = self.optimizer._extract_leftovers_features(self.test_leftovers)
    ...
    def test_find_optimal_assignments(self):
        ...
        pieces = self.optimizer._extract_pieces_features(self.test_dxf_data)
        leftover_features = self.optimizer._extract_leftovers_features(self.test_leftovers)
        
        assignments = self.optimizer._find_optimal_assignments(pieces, leftover_features)
