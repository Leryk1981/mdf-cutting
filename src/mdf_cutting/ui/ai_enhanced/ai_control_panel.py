...
class ImprovedAIControlPanel(ttk.Frame):
    ...
    def __init__(self, parent: tk.Widget, ai_service: AIIntegrationService, 
                 app: DesktopApp):
    ...
    def setup_ui(self):
        ...
        self.log_text.tag_config('info', foreground=AIStyles.COLORS['text_primary'])
        self.log_text.tag_config('success', foreground=AIStyles.COLORS['success'])
        self.log_text.tag_config('warning', foreground=AIStyles.COLORS['warning'])
        ...
        for col in columns:
            ...
            self.suggestions_tree.column(col, 
                                      width=100,
                                      minwidth=80,
        ...
        self.suggestions_tree.tag_configure(
            'high_confidence', 
            background=AIStyles.COLORS['success'] + '20'
        )
        self.suggestions_tree.tag_configure(
            'medium_confidence', 
            background=AIStyles.COLORS['warning'] + '30'
        )
        self.suggestions_tree.tag_configure(
            'low_confidence', 
            background=AIStyles.COLORS['danger'] + '20'
        )
    ...
    def process_messages(self):
        ...
        try:
            while not self.message_queue.empty():
                ...
                if message['type'] == 'log':
                    self.log_message(message['text'], message.get('tag', 'info'))
    ...
    def optimize_with_ai(self):
        ...
        if self.processing_thread and self.processing_thread.is_alive():
            self.toast_notification.show("AI уже обрабатывает запрос", "warning")
            return
        ...
        self.processing_thread = threading.Thread(
            target=self._perform_optimization,
            args=(order_data,),
            daemon=True
        )
    ...
    def _perform_optimization(self, order_data: Dict[str, Any]):
        ...
        try:
            ...
            if result.get("status") == "success":
                self.send_message('update_suggestions', 
                                suggestions=result.get("ai_enhancements", []))
                self.after(0, lambda: self.app.update_dxf_display(result.get("final_dxf")))
                
                # Уведомление об успехе
                suggestions_count = len(result.get("ai_enhancements", []))
                self.send_message('toast', 
                                f"Найдено {suggestions_count} предложений для улучшения", 
                                'success')
            else:
                self.send_message('log', f"Ошибка: {result.get('error', 'Unknown')}", 'error')
                self.send_message('toast', f"Ошибка оптимизации: {result.get('error')}", 'error')
            ...
            self.send_message('toast', f"Критическая ошибка: {error_msg}", 'error')
    ...
    def on_optimization_completed(self, result: Dict[str, Any]):
        ...
        self.status_indicator.config(text="● AI готов", 
                                   foreground=AIStyles.COLORS['success'])
    
    def on_optimization_failed(self, error_msg: str):
        """Обработка ошибки оптимизации"""
        self.set_processing_state(False)
        self.status_indicator.config(text="● Ошибка AI", 
                                   foreground=AIStyles.COLORS['danger'])
    ...
    def set_processing_state(self, processing: bool):
        ...
        if processing:
            ...
            self.status_indicator.config(text="● AI работает", 
                                       foreground=AIStyles.COLORS['warning'])
    ...
    def optimize_with_leftovers(self):
        ...
        self.send_message('log', f'Использование {len(available_leftovers)} остатков', 'info')
    ...
    def _perform_leftover_optimization(self, order_data: Dict, leftovers: List[Dict]):
        """Оптимизация с остатками в фоне"""
        try:
            result = self.ai_service.process_with_leftovers(order_data, leftovers)
            
            savings = result.get("material_savings", 0)
            self.send_message('log', f'Экономия материала: {savings:.1f}%', 'success')
            self.send_message('status_bar', f'Готово. Экономия: {savings:.1f}%')
            self.send_message('toast', f'Сэкономлено {savings:.1f}% материала!', 'success')
            
            self.after(0, self.on_leftover_optimization_completed, result)
            
        except Exception as e:
            error_msg = str(e)
            self.send_message('log', f'Ошибка оптимизации с остатками: {error_msg}', 'error')
            self.after(0, self.on_optimization_failed, error_msg)
    ...
    def apply_selected_suggestion(self):
        ...
        if not selected_items:
            self.toast_notification.show("Выберите предложение для применения", "warning")
            return
        ...
        if suggestion:
            self.send_message('log', f'Применение предложения: {suggestion.get("correction_type")}', 'info')
            
            # Применение предложения
            if hasattr(self.app, 'apply_correction'):
                self.app.apply_correction(suggestion)
                self.send_message('toast', 'Предложение применено успешно!', 'success')
                
                # Удаление из списка
                self.suggestions_tree.delete(item_id)
            else:
                self.toast_notification.show("Метод apply_correction не найден", "error")
    ...
    def reject_suggestion(self):
        ...
        if selected_items:
            ...
            self.send_message('log', 'Предложение отклонено пользователем', 'warning')
    ...
    def update_suggestions(self, suggestions: List[Dict[str, Any]]):
        ...
        for i, suggestion in enumerate(suggestions):
            ...
            if confidence >= 0.8:
                ...
                self.suggestions_tree.item(item_id, tags=('medium_confidence',))
            else:
    ...
    def getSuggestionData(self, item_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return self.current_suggestions[index] if index < len(self.current_suggestions) else None
    ...
    def get_current_order_data(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            pieces_data = self.app.get_pieces_data() if hasattr(self.app, 'get_pieces_data') else []
            ...
            return {
                ...
                "material_code": getattr(self.app, 'get_material_code', lambda: "MDF16")(),
                "thickness": getattr(self.app, 'get_material_thickness', lambda: 16.0)(),
                "pieces": pieces_data
            }
        except Exception as e:
            self.send_message('log', f"Ошибка получения данных заказа: {str(e)}", 'error')
            return None
    ...
    def setup_suggestion_menu(self):
        ...
        self.suggestion_menu.add_command(label="✓ Применить", 
                                       command=self.apply_selected_suggestion)
        self.suggestion_menu.add_command(label="📋 Подробнее", 
                                       command=self.show_suggestion_details)
        self.suggestion_menu.add_separator()
        self.suggestion_menu.add_command(label="✗ Отклонить", 
                                       command=self.reject_suggestion)
        
        self.suggestions_tree.bind("<Button-3>", self.show_suggestion_menu)
        self.suggestions_tree.bind("<Double-1>", 
                                 lambda e: self.apply_selected_suggestion())
    ...
    def on_ai_mode_changed(self):
        ...
        status_text = "● AI готов" if self.ai_mode_var.get() else "● AI выключен"
        status_color = AIStyles.COLORS['success'] if self.ai_mode_var.get() else AIStyles.COLORS['danger']
...
class DetailDialog(tk.Toplevel):
    ...
    def setup_dialog(self):
        ...
        details = [
            ("Тип корректировки:", self.suggestion_data.get('correction_type', 'N/A')),
            ("ID детали:", self.suggestion_data.get('piece_id', 'N/A')),
            ("Уверенность AI:", f"{self.suggestion_data.get('confidence', 0):.2f}"),
            ("Потенциал улучшения:", f"{self.suggestion_data.get('expected_improvement', 0):.3f}")
        ]
        ...
        params_content = '\n'.join([
            f"  {key}: {value}" 
            for key, value in self.suggestion_data.get('parameters', {}).items()
        ])
