...
class CuttingApp:
    ...
    def setup_ai_integration(self):
        ...
        for child in self.root.winfo_children():
            if isinstance(child, ttk.LabelFrame) or isinstance(
                child, ttk.Frame
            ):
                child.pack_forget()
        ...
        # AI вкладка
        self.ai_panel = ImprovedAIControlPanel(
            self.notebook, self.ai_service, self
        )
    ...
    def add_ai_buttons(self):
        ...
        for child in self.main_frame.winfo_children():
            if isinstance(child, ttk.LabelFrame) and "Действия" in child.cget(
                "text"
            ):
                action_frame = child
                break
    ...
    def quick_ai_optimize(self):
        ...
        self.ai_status_label.config(
            text="●", foreground=AIStyles.COLORS["warning"]
        )
    ...
    def get_pieces_data(self):
        ...
        try:
            if (
                ...
                and self.data_manager.details_data is not None
            ):
    ...
    def get_material_code(self):
        ...
        try:
            if (
                hasattr(self, "data_manager")
                and self.data_manager.materials_data is not None
            ):
                return self.data_manager.materials_data.iloc[0][
                    "material_code"
                ]
            return "MDF16"
        except Exception:
    ...
    def get_material_thickness(self):
        ...
        try:
            if (
                ...
                return float(
                    self.data_manager.materials_data.iloc[0]["thickness"]
                )
            return 16.0
        except Exception:
            return 16.0
    ...
    def apply_correction(self, suggestion_data):
        ...
        logger.info(
            f"Применена корректировка: {suggestion_data.get('correction_type')}"
        )
