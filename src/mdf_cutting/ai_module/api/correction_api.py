...
from ..core.leftover_optimizer import (
...
class CorrectionAPI:
    ...
    def get_corrections(
        self, dxf_path: Path, order_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            optimization_features = (
                self.optimization_extractor.extract_features(
                    dxf_data, order_data
                )
            )
            ...
            model_input = self._prepare_model_input(
                geometry_features, optimization_features
            )
            ...
            self.logger.info(
                f"Successfully processed {dxf_path} in {processing_time:.2f}s"
            )
            return response
    ...
    def batch_process(
        ...
        for request in requests:
            try:
                ...
                results.append(
                    {"status": "error", "error": str(e), "request": request}
                )
    ...
    def apply_corrections(
        ...
        try:
            ...
            self.logger.info(
                f"Corrections applied successfully. Saved to {output_path}"
            )
    ...
    def _load_model(
        self,
    ) -> Union[EnsembleCorrectionModel, HybridCorrectionModel]:
    ...
    def _prepare_model_input(
        self, geometry_features: Dict, optimization_features: Dict
    ...
    def _process_ensemble_output(self, outputs: Dict) -> Dict[str, Any]:
        ...
        type_probs = (
            torch.softmax(outputs["correction_type"], dim=-1).cpu().numpy()[0]
        )
        ...
        return {
            ...
            "type_probabilities": {
                types[i]: float(type_probs[i]) for i in range(len(types))
            },
        }

    def _generate_corrections(
        self, predictions: Dict, dxf_data: Dict
    ) -> List[Dict[str, Any]]:
        ...
        if (
            ...
            and predictions.get("improvement_potential", 0) > 0.1
        ):
                ...
                if entity.get("type") in ["LWPOLYLINE", "POLYLINE"]:
                    ...
                    if self._is_problematic_piece(entity, dxf_data):
                        correction = {
                            ...
                            "expected_improvement": predictions.get(
    ...
    def optimize_with_leftovers(
        self,
        dxf_path: Path,
        order_data: Dict[str, Any],
        force_new_sheets: bool = False,
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            dxf_data = self.data_loader.load_dxf(dxf_path)
            self.geometry_extractor.extract_features(
                dxf_data
            )

            # 2. Анализ остатков
            leftover_features = (
                self.leftover_extractor.extract_leftover_features(
                    dxf_data, self.leftovers_db
                )
            )

            # 3. Определение стратегии
            leftover_features["optimization_strategy"]

            # Если принудительно требуются новые листы или нет подходящих остатков
            if (
                force_new_sheets
                or len(
                    leftover_features["material_efficiency"][
                        "suitable_leftovers"
                    ]
                )
                == 0
            ...
            result = {
                ...
                "suitable_leftovers": leftover_features[
            ...
            self.logger.info(
                f"Optimized with leftovers in {processing_time:.2f}s"
            )
            return result
    ...
    def update_leftovers_db(self, new_leftovers: List[Dict[str, Any]]):
        ...
        try:
            ...
            self.logger.info(
                f"Added {len(new_leftovers)} new leftovers to database"
            )
    ...
    def get_leftovers_usage_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            used_leftovers = sum(
                1 for l in self.leftovers_db if l.usage_count > 0
            )
            ...
            return {
                ...
                "usage_rate": (
                    used_leftovers / total_leftovers
    ...
    def _init_leftover_optimizer(self) -> LeftoverOptimizer:
        ...
        try:
            ...
            if model_path.exists():
                ...
                self.logger.warning(
                    "Could not load trained leftover optimizer, using untrained model"
                )
