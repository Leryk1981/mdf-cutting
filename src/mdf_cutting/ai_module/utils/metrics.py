...
class CorrectionMetrics:
    ...
    def _evaluate_predictions(
        ...
        try:
            ...
            if "correction_params" in predictions:
                ...
                metrics["params_range"] = float(
                    np.max(params) - np.min(params)
                )

                # Оценка разумности параметров
                metrics["params_reasonableness"] = (
                    self._assess_params_reasonableness(params)
                )
            ...
            if "confidence" in predictions:
                ...
                metrics["confidence_reliability"] = (
                    self._assess_confidence_reliability(confidence)
                )

            # Оценка типа корректировки
            if "correction_type" in predictions:
                correction_type = predictions["correction_type"]
                metrics["correction_type_confidence"] = (
                    self._assess_type_confidence(correction_type)
                )
    ...
    def _evaluate_geometric_improvement(
        self,
        original_data: Dict[str, Any],
        predictions: Dict[str, Any],
        features: Dict[str, Any],
        ...
        try:
            ...
            if "improvement_score" in predictions:
                ...
                expected_utilization = original_utilization + (
                    improvement_potential * 20
                )  # До 20% улучшения
                expected_waste = max(
                    0, original_waste - (improvement_potential * 15)
                )  # До 15% снижения отходов

                metrics["expected_utilization_improvement"] = (
                    expected_utilization - original_utilization
                )
                metrics["expected_waste_reduction"] = (
                    original_waste - expected_waste
                )
                metrics["improvement_potential"] = float(improvement_potential)

            # Оценка геометрической сложности
            metrics["geometric_complexity"] = (
                self._assess_geometric_complexity(original_data)
            )

            # Оценка пространственной эффективности
            metrics["spatial_efficiency"] = self._assess_spatial_efficiency(
                features
            )
    ...
    def _evaluate_optimization_quality(
        ...
        try:
            ...
            feasibility_score = self._assess_technical_feasibility(
                predictions, features
            )
            metrics["technical_feasibility"] = feasibility_score

            # Оценка экономической эффективности
            economic_score = self._assess_economic_efficiency(
                predictions, features
            )
    ...
    def _assess_geometric_complexity(
    ...
    def _assess_spatial_efficiency(self, features: Dict[str, Any]) -> float:
        ...
        try:
            ...
            distribution_evenness = spatial_features.get(
            ...
            logger.warning(
                f"Ошибка оценки пространственной эффективности: {e}"
            )
            return 0.5

    def _assess_technical_feasibility(
        self, predictions: Dict[str, Any], features: Dict[str, Any]
    ) -> float:
        """Оценка технологической осуществимости."""
        try:
            ...
            if "correction_type" in predictions:
                ...
                if correction_type == "custom":
                    feasibility_score *= (
                        0.8  # Сложные корректировки менее осуществимы
                    )
            ...
            logger.warning(
                f"Ошибка оценки технологической осуществимости: {e}"
            )
            return 0.5

    def _assess_economic_efficiency(
        self, predictions: Dict[str, Any], features: Dict[str, Any]
    ) -> float:
        """Оценка экономической эффективности."""
        try:
    ...
    def _calculate_overall_score(
        ...
        try:
            ...
            geometric_score = (
                ...
                + (1.0 - geometric_metrics.get("geometric_complexity", 0))
                * 0.3
            )

            # Оценка оптимизации
            optimization_score = optimization_metrics.get(
