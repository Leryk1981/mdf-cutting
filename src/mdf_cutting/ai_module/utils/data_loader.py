            ...
            for key in keys:
                ...
                if 'features' in f[key]:
                    ...
                    for feature_name in features_group.keys():
                        sample[f'features_{feature_name}'] = features_group[feature_name][:]
                
                # Загружаем цели
                if 'targets' in f[key]:
                    targets_group = f[key]['targets']
                    for target_name in targets_group.keys():
                        sample[f'targets_{target_name}'] = targets_group[target_name][:]
                
                # Загружаем метаданные
                if 'metadata' in f[key]:
                    metadata_group = f[key]['metadata']
                    for meta_name in metadata_group.keys():
                        sample[f'metadata_{meta_name}'] = metadata_group[meta_name][:]
...
class DataPreprocessor:
    ...
    def fit(self, dataset: CuttingMapDataset):
        ...
        logger.info(f"Скейлеры обучены для {len(self.scalers)} типов признаков")
    ...
    def transform(self, features: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        for feature_name, feature_tensor in features.items():
            ...
            if feature_type in self.scalers:
                ...
                scaled_feature = self.scalers[feature_type].transform(feature_np)
                transformed_features[feature_name] = torch.FloatTensor(scaled_feature)
            else:
                # Без трансформации
...
class MLDataLoader:
    ...
    def create_dataset(self, data_path: str, preprocess: bool = True) -> CuttingMapDataset:
        """Создание датасета"""
        ...
        if preprocess:
            ...
            transform = lambda features: self.preprocessor.transform(features)
            
            # Создание нового датасета с трансформацией
            dataset = CuttingMapDataset(data_path, transform=transform)
        
        return dataset
    
    def create_data_loaders(self, dataset: CuttingMapDataset, 
                          batch_size: int = 32,
        ...
        logger.info(f"Созданы DataLoader'ы: train={len(train_loader)}, val={len(val_loader)}, test={len(test_loader)}")
    ...
    def prepare_features_for_model(self, dxf_data: Dict[str, Any], 
                                 order_data: Dict[str, Any]) -> Dict[str, torch.Tensor]:
        """Подготовка признаков для модели"""
        from src.mdf_cutting.ai_module.features.geometry import GeometryFeatureExtractor
        from src.mdf_cutting.ai_module.features.optimization import OptimizationFeatureExtractor
        from src.mdf_cutting.ai_module.features.material import MaterialFeatureExtractor
        ...
        optimization_features = optimization_extractor.extract_features(dxf_data, order_data)
        
        # Признаки материалов
        material_features = material_extractor.extract_features(order_data, order_data)
            ...
            if isinstance(feature_value, dict):
                ...
                for sub_name, sub_value in feature_value.items():
                    if isinstance(sub_value, (int, float)):
                        tensor_features[f"{feature_name}_{sub_name}"] = torch.FloatTensor([sub_value])
                    elif isinstance(sub_value, list):
                        tensor_features[f"{feature_name}_{sub_name}"] = torch.FloatTensor(sub_value)
            elif isinstance(feature_value, (int, float)):
                tensor_features[feature_name] = torch.FloatTensor([feature_value])
        ...
        grouped_features = {
            ...
            'optimization': self._extract_optimization_features(tensor_features)
        }
        
        return grouped_features
    
    def _extract_global_features(self, features: Dict[str, torch.Tensor]) -> torch.Tensor:
        """Извлечение глобальных признаков"""
        global_keys = [k for k in features.keys() if 'global' in k or 'utilization' in k or 'waste' in k]
    ...
    def _extract_local_features(self, features: Dict[str, torch.Tensor]) -> torch.Tensor:
        """Извлечение локальных признаков"""
        local_keys = [k for k in features.keys() if 'local' in k or 'piece' in k or 'compactness' in k]
    ...
    def _extract_spatial_features(self, features: Dict[str, torch.Tensor]) -> torch.Tensor:
        """Извлечение пространственных признаков"""
        spatial_keys = [k for k in features.keys() if 'spatial' in k or 'quadrant' in k or 'distribution' in k]
    ...
    def _extract_topological_features(self, features: Dict[str, torch.Tensor]) -> torch.Tensor:
        """Извлечение топологических признаков"""
        topo_keys = [k for k in features.keys() if 'topological' in k or 'graph' in k or 'connectivity' in k]
    ...
    def _extract_optimization_features(self, features: Dict[str, torch.Tensor]) -> torch.Tensor:
        """Извлечение признаков оптимизации"""
        optim_keys = [k for k in features.keys() if 'optimization' in k or 'cutting' in k or 'efficiency' in k]
