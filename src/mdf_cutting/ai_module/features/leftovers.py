...
class LeftoverFeatureExtractor:
    ...
    def extract_leftover_features(self, dxf_data: Dict, available_leftovers: List[Leftover]) -> Dict[str, Any]:
        """Извлечь признаки, связанные с остатками."""
        try:
            return {
                "material_efficiency": self._calculate_material_efficiency(dxf_data, available_leftovers),
                "leftover_suitability": self._evaluate_leftover_suitability(dxf_data, available_leftovers),
                "new_leftovers_potential": self._predict_new_leftovers(dxf_data),
                "optimization_strategy": self._determine_optimization_strategy(dxf_data, available_leftovers)
            }
        except Exception as e:
            logger.error(f"Error extracting leftover features: {e}")
            return self._get_default_features()
    
    def _calculate_material_efficiency(self, dxf_data: Dict, leftovers: List[Leftover]) -> Dict[str, Any]:
        """Оценка эффективности использования материала с учетом остатков."""
        try:
            # Общая площадь заказа
            order_area = sum(self._get_entity_area(e) for e in dxf_data.get("entities", []))
            ...
            for leftover in leftovers:
                compatibility_score = self._calculate_compatibility(dxf_data, leftover)
            ...
            coverage_ratio = leftover_coverage / order_area if order_area > 0 else 0
            
            return {
                "total_leftover_area_available": sum(l.area for l in leftovers),
                "suitable_leftovers_count": len(suitable_leftovers),
                "leftover_coverage_ratio": coverage_ratio,
                "material_savings_potential": coverage_ratio * 100,  # в процентах
                "new_sheet_required": coverage_ratio < 0.8,  # нужны новые листы
    ...
    def _evaluate_leftover_suitability(self, dxf_data: Dict, leftovers: List[Leftover]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for leftover in leftovers:
                ...
                geometric_score = self._calculate_geometric_suitability(order_pieces, leftover)
                ...
                if total_score > 0.3:  # Минимальный порог
                    suitable_leftovers.append({
                        ...
                        "optimization_potential": self._estimate_optimization_potential(order_pieces, leftover),
                        "area": leftover.area,
                        "material_code": leftover.material_code
                    })
            
            # Сортировка по пригодности
            suitable_leftovers.sort(key=lambda x: x["suitability_score"], reverse=True)
        ...
        try:
            ...
            for sheet in sheet_polygons:
                ...
                if remaining_area > self.minimum_leftover_area:
                    ...
                    leftover_shape = self._predict_leftover_shape(sheet, pieces, remaining_area)
                    
                    predicted_leftovers.append({
                        "geometry": leftover_shape,
                        "area": remaining_area,
                        "bounding_box": self._calculate_bounding_box(leftover_shape),
                        "utilization_potential": self._calculate_utilization_potential(remaining_area),
                        "storage_efficiency": self._calculate_storage_efficiency(leftover_shape),
                        "priority": self._calculate_priority(remaining_area, leftover_shape)
                    })
            
            # Сортировка по приоритету
            predicted_leftovers.sort(key=lambda x: x["utilization_potential"], reverse=True)
            return predicted_leftovers
        except Exception as e:
            logger.error(f"Error predicting new leftovers: {e}")
            return []
    
    def _determine_optimization_strategy(self, dxf_data: Dict, leftovers: List[Leftover]) -> Dict[str, Any]:
        """Определение стратегии оптимизации на основе доступных остатков."""
        try:
            # Анализ состава заказа
            entities = dxf_data.get("entities", [])
            has_large_pieces = any(self._get_entity_area(e) > 100000 for e in entities)
            has_small_pieces = any(self._get_entity_area(e) < 10000 for e in entities)
            ...
            strategy = {
                ...
                "leftover_first": len(large_leftovers) > 0,  # приоритет остаткам
            ...
            if has_large_pieces and len(large_leftovers) == 0:
                strategy["primary_strategy"] = "new_sheets"  # нужны новые листы
            elif has_small_pieces and len(small_leftovers) > 3:
                strategy["primary_strategy"] = "leftover_efficient"  # эффективное использование остатков
    ...
    def _calculate_compatibility(self, dxf_data: Dict, leftover: Leftover) -> float:
        ...
        try:
            ...
            max_piece_height = max(self._get_entity_height(e) for e in entities)
            
            # Базовая пространственная совместимость
            spatial_fit = min(
                leftover.width / max_piece_width if max_piece_width > 0 else 1,
                leftover.height / max_piece_height if max_piece_height > 0 else 1
            )
    ...
    def _calculate_geometric_suitability(self, order_pieces: List, leftover: Leftover) -> float:
    ...
    def _calculate_recency_score(self, leftover: Leftover) -> float:
        ...
        try:
            ...
            creation_date = datetime.fromisoformat(leftover.creation_date.replace('Z', '+00:00'))
            days_old = (datetime.now() - creation_date).days
            
            # Экспоненциальное затухание
            recency_score = np.exp(-days_old / 30)  # 30 дней - период полураспада
    ...
    def _estimate_optimization_potential(self, order_pieces: List, leftover: Leftover) -> float:
        """Оценка потенциала оптимизации для данного остатка."""
        try:
            # Упрощенная оценка на основе площади и формы
            area_ratio = leftover.area / sum(p.get("area", 0) for p in order_pieces)
            aspect_ratio = leftover.width / leftover.height if leftover.height > 0 else 1
    ...
    def _predict_leftover_shape(self, sheet: Dict, pieces: List, remaining_area: float) -> Dict[str, Any]:
        """Предсказание формы остатка."""
        try:
            # Упрощенная модель - прямоугольный остаток
            sheet_width = sheet.get("width", 1000)
            sheet_height = sheet.get("height", 1000)
    ...
    def _calculate_bounding_box(self, shape: Dict) -> Tuple[float, float, float, float]:
    ...
    def _matches_material_spec(self, dxf_data: Dict, leftover: Leftover) -> bool:
        ...
        return (leftover.material_code == order_material and 
                abs(leftover.thickness - order_thickness) < 0.1)
    ...
    def _get_default_features(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "optimization_strategy": {"primary_strategy": "mixed", "leftover_first": False}
        }
