...
try:
    ...
    logging.warning(
        "Shapely не установлен. Геометрические признаки будут ограничены."
    )
...
class GeometryFeatureExtractor:
    ...
    def __init__(self):
        ...
        if not SHAPELY_AVAILABLE:
            logger.warning(
                "Shapely недоступен. Используется упрощенный режим."
            )
    ...
    def _parse_pieces_polygons(self, dxf_data: Dict) -> List[Polygon]:
        ...
        for entity in dxf_data.get("entities", []):
            try:
                ...
                logger.warning(
                    f"Ошибка парсинга сущности {entity.get('type', 'unknown')}: {e}"
                )
                continue
    ...
    def _extract_global_features(
        self, pieces: List[Polygon], sheet: Polygon
    ) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "utilization_rate": (
                    (pieces_area / sheet_area) * 100 if sheet_area > 0 else 0
                ),
                "pieces_complexity": self._calculate_complexity(pieces),
                "sheet_coverage": self._calculate_coverage(
                    union_pieces, sheet
                ),
                "avg_piece_area": pieces_area / len(pieces) if pieces else 0,
                "piece_area_variance": (
                    np.var([p.area for p in pieces]) if len(pieces) > 1 else 0
                ),
            }
        except Exception as e:
                ...
                local_features.append(
                    {
                        ...
                        "compactness": (
                            (4 * np.pi * piece.area) / (piece.length**2)
                            if piece.length > 0
                            else 0
                        ),
                        ...
                        "rectilinearity": self._calculate_rectilinearity(
                            piece
                        ),
                        "is_near_edge": min_dist_to_edge < 50,  # Близко к краю
                        "neighbor_count": len(
                            [d for d in neighbor_distances if d < 20]
                        ),  # Количество соседей
                    }
                )
        except Exception as e:
    ...
    def _extract_spatial_features(
        ...
        try:
            ...
            distribution_evenness = (
                1 - (np.std(list(quadrants.values())) / ideal_per_quadrant)
                if ideal_per_quadrant > 0
                else 0
            )
            ...
            return {
                ...
                "spatial_dispersion": self._calculate_spatial_dispersion(
                    centroids, sheet
                ),
        ...
        try:
            ...
            if G.number_of_nodes() > 0:
                graph_features = {
                    ...
                    "num_connected_components": nx.number_connected_components(
                        G
                    ),
                    "avg_clustering": (
    ...
    def _extract_simple_features(self, dxf_data: Dict) -> Dict:
        ...
        # Подсчет сущностей
        polyline_count = len(
            [e for e in entities if e["type"] in ["LWPOLYLINE", "POLYLINE"]]
        )
    ...
    def _calculate_coverage(
        self, union_pieces: Polygon, sheet: Polygon
    ) -> float:
    ...
    def _calculate_spatial_dispersion(
        self, centroids: List[Point], sheet: Polygon
    ) -> float:
