...
class MaterialFeatureExtractor:
    ...
    def __init__(self):
        ...
        self.material_properties = {
            "MDF": {
                ...
                "standard_sizes": [(2440, 1220), (2800, 2070), (3660, 1830)]  # мм
            },
            "HDF": {
                "density": 850,
                "hardness": 0.9,
    ...
    def extract_features(self, material_data: Dict, order_data: Dict) -> Dict[str, Any]:
        """
        ...
        """
        try:
            basic_features = self._extract_basic_features(material_data, order_data)
            technical_features = self._extract_technical_features(material_data, order_data)
            processing_features = self._extract_processing_features(material_data, order_data)
    ...
    def _extract_basic_features(self, material_data: Dict, order_data: Dict) -> Dict:
        """Базовые признаки материала."""
        material_type = material_data.get("type", order_data.get("material_code", "MDF"))
        thickness = material_data.get("thickness", order_data.get("thickness", 16.0))
        quantity = order_data.get("quantity", 1)
        
        # Получаем свойства материала
        properties = self.material_properties.get(material_type.upper(), 
                                                 self.material_properties["MDF"])
        ...
        return {
            ...
            "is_standard_thickness": self._is_standard_thickness(thickness, material_type),
            "is_standard_size": self._is_standard_size(sheet_size, material_type)
        }
    
    def _extract_technical_features(self, material_data: Dict, order_data: Dict) -> Dict:
        ...
        return {
            ...
            "density_factor": properties["density"] / 750.0,  # относительно МДФ
            "hardness_factor": properties["hardness"],
            "material_strength": self._calculate_material_strength(properties, thickness),
            "cutting_difficulty": self._calculate_cutting_difficulty(properties, thickness),
            "surface_finish_quality": self._calculate_surface_quality(properties, thickness)
        }
    
    def _extract_processing_features(self, material_data: Dict, order_data: Dict) -> Dict:
        ...
        sheet_size = self._get_sheet_size(material_type, material_data)
        processing_time = self._estimate_processing_time(sheet_size, thickness, properties)
        
        # Качество обработки
        edge_quality = self._estimate_edge_quality(properties, thickness)
        dimensional_accuracy = self._estimate_dimensional_accuracy(properties, thickness)
        ...
        return {
            ...
            "cutting_force_factor": self._calculate_cutting_force_factor(properties, thickness),
            "tool_wear_factor": self._calculate_tool_wear_factor(properties, thickness),
            "dust_generation_factor": self._calculate_dust_generation_factor(properties),
            "noise_level_factor": self._calculate_noise_level_factor(properties, thickness),
            "requires_special_tooling": self._requires_special_tooling(material_type, thickness),
            "post_processing_required": self._requires_post_processing(material_type, thickness)
        }
    
    def _get_sheet_size(self, material_type: str, material_data: Dict) -> Tuple[float, float]:
    ...
    def _is_standard_thickness(self, thickness: float, material_type: str) -> bool:
        ...
        return thickness in standard_thicknesses and thickness_range[0] <= thickness <= thickness_range[1]
    
    def _is_standard_size(self, sheet_size: Tuple[float, float], material_type: str) -> bool:
    ...
    def _calculate_material_strength(self, properties: Dict, thickness: float) -> float:
    ...
    def _calculate_cutting_difficulty(self, properties: Dict, thickness: float) -> float:
        ...
        difficulty = (0.4 * hardness_factor + 
                     0.3 * thickness_factor + 
                     0.3 * density_factor)
        
        return min(difficulty, 1.0)
    
    def _calculate_surface_quality(self, properties: Dict, thickness: float) -> float:
        """Расчет качества поверхности."""
        base_quality = properties["surface_quality"]
        thickness_factor = min(thickness / 16.0, 1.5)  # Толщина влияет на качество
        
        return min(base_quality * thickness_factor, 1.0)
    
    def _estimate_processing_time(self, sheet_size: Tuple[float, float], 
                                thickness: float, properties: Dict) -> float:
        ...
        processing_time = (area / 1000000) * base_time_per_sqm * thickness_factor * material_factor
        
        return processing_time
    
    def _estimate_edge_quality(self, properties: Dict, thickness: float) -> float:
    ...
    def _estimate_dimensional_accuracy(self, properties: Dict, thickness: float) -> float:
    ...
    def _calculate_cutting_force_factor(self, properties: Dict, thickness: float) -> float:
        """Расчет фактора силы резания."""
        density_factor = properties["density"] / 750.0
        hardness_factor = properties["hardness"]
        thickness_factor = thickness / 16.0
        
        force_factor = (0.4 * density_factor + 
                       0.4 * hardness_factor + 
                       0.2 * thickness_factor)
        
        return min(force_factor, 1.5)
    
    def _calculate_tool_wear_factor(self, properties: Dict, thickness: float) -> float:
    ...
    def _calculate_noise_level_factor(self, properties: Dict, thickness: float) -> float:
    ...
    def _requires_special_tooling(self, material_type: str, thickness: float) -> bool:
        """Требуется ли специальный инструмент."""
        # Специальный инструмент для твердых материалов или большой толщины
        properties = self.material_properties.get(material_type.upper(), 
                                                 self.material_properties["MDF"])
        
        return (properties["hardness"] > 0.8 or 
                thickness > 25 or 
                material_type.upper() in ["HDF", "массив"])
    
    def _requires_post_processing(self, material_type: str, thickness: float) -> bool:
        """Требуется ли постобработка."""
        # Постобработка для материалов с низким качеством поверхности
        properties = self.material_properties.get(material_type.upper(), 
                                                 self.material_properties["MDF"])
        
        return (properties["surface_quality"] < 0.7 or 
                material_type.upper() in ["ДСП", "массив"])
