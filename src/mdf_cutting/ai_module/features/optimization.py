...
class OptimizationFeatureExtractor:
    ...
    def extract_features(
        self, dxf_data: Dict, order_data: Dict
    ) -> Dict[str, Any]:
        """
        ...
        """
        try:
            material_features = self._extract_material_features(
                dxf_data, order_data
            )
    ...
    def _extract_material_features(
        self, dxf_data: Dict, order_data: Dict
    ) -> Dict:
        ...
        return {
            ...
            "material_cost_factor": self._calculate_material_cost_factor(
    ...
    def _extract_cutting_features(self, dxf_data: Dict) -> Dict:
        ...
        cut_lines = [
            e
            for e in dxf_data.get("entities", [])
        ...
        cutting_time = self._estimate_cutting_time(
            total_cut_length, complex_cuts
        )

        # Эффективность резки
        cutting_efficiency = self._calculate_cutting_efficiency(
            cut_lines, dxf_data
        )
        ...
        return {
            ...
            "complex_cut_ratio": (
                complex_cuts / len(cut_lines) if cut_lines else 0
            ),
            "avg_cut_length": (
                total_cut_length / len(cut_lines) if cut_lines else 0
            ),
    ...
    def _extract_efficiency_features(self, dxf_data: Dict) -> Dict:
        ...
        utilization_rate = (
            (total_area / sheet_area) * 100 if sheet_area > 0 else 0
        )
        ...
        return {
            ...
            "large_pieces_ratio": (
                len([a for a in areas if a > np.median(areas)]) / len(areas)
                if areas
                else 0
            ),
            "small_pieces_ratio": (
                len([a for a in areas if a < np.percentile(areas, 25)])
                / len(areas)
                if areas
                else 0
            ),
    ...
    def _calculate_material_cost_factor(
        ...
        thickness_factor = (
            thickness / 16.0
        )  # относительно стандартной толщины 16мм
    ...
    def _estimate_cutting_time(
        self, total_length: float, complex_cuts: int
    ) -> float:
    ...
    def _calculate_cutting_efficiency(
        self, cut_lines: List[Dict], dxf_data: Dict
    ) -> float:
    ...
    def _analyze_waste_patterns(self, dxf_data: Dict) -> Dict:
        ...
        waste_percentage = (
            (waste_area / sheet_area) * 100 if sheet_area > 0 else 0
        )
    ...
    def _estimate_optimization_potential(self, dxf_data: Dict) -> float:
        ...
        piece_count_factor = min(
            len(pieces) / 20, 1.0
        )  # Больше деталей = больше потенциал

        # Корректировка на основе размера деталей
        avg_area = np.mean(areas)
        size_factor = min(
            avg_area / 10000, 1.0
        )  # Меньшие детали = больше потенциал
