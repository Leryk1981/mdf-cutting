...
try:
    ...
    logging.warning(
        "MLflow не установлен. Логирование экспериментов будет ограничено."
    )
    ...
    def _train_epoch(self, train_loader: DataLoader) -> float:
        ...
        for batch in tqdm(train_loader, desc="Training"):
            ...
            params_loss = self.params_criterion(
                outputs["correction_params"], targets["correction_params"]
            )

            confidence_loss = self.confidence_criterion(
                outputs["improvement_score"], targets["confidence"]
            )

            type_loss = self.type_criterion(
                outputs["correction_type"], targets["correction_type"]
            )
    ...
    def _validate_epoch(
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                loss = (
                    0.5 * params_loss + 0.3 * confidence_loss + 0.2 * type_loss
                )
                ...
                all_confidences.extend(
                    outputs["improvement_score"].cpu().numpy()
                )
        ...
        # Расчет метрик
        confidence_acc = (
            np.mean(np.round(all_confidences) == all_targets)
            if all_confidences
            else 0.0
        )
        avg_params_mae = (
            np.mean(all_params_errors) if all_params_errors else 0.0
        )
...
class EnsembleTrainer:
    ...
    def _train_individual_models(
        ...
        for i, (model, optimizer) in enumerate(
            zip(self.ensemble_model.models, self.optimizers)
        ):
            model.train()
    ...
    def _validate_ensemble(
        ...
        metrics = {
            "confidence_acc": confidence_acc,
            "ensemble_weights": outputs["ensemble_weights"]
            .cpu()
            .numpy()
            .tolist(),
        }
...
class ModelEvaluator:
    ...
    def _calculate_metrics(
        ...
        for pred, target in zip(predictions, targets):
            all_correction_params.append(
                pred["correction_params"].cpu().numpy()
            )
            all_correction_types.append(pred["correction_type"].cpu().numpy())
            all_improvement_scores.append(
                pred["improvement_score"].cpu().numpy()
            )
        ...
        type_accuracy = np.mean(
            np.argmax(pred_types, axis=1) == np.argmax(target_types, axis=1)
        )

        confidence_mae = np.mean(np.abs(pred_scores - target_confidences))

        # Дополнительные метрики
        params_correlation = np.corrcoef(
            pred_params.flatten(), target_params.flatten()
        )[0, 1]
