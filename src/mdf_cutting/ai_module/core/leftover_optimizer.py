...
class LeftoverOptimizationModel(nn.Module):
    ...
    def forward(
        self, leftover_features: torch.Tensor, piece_features: torch.Tensor
...
class LeftoverOptimizer:
    ...
    def __init__(
        self, model: LeftoverOptimizationModel, config: Dict[str, Any]
    ):
        self.model = model
        self.config = config
    ...
    def optimize_layout(
        ...
        try:
            ...
            leftover_features = self._extract_leftovers_features(
                available_leftovers
            )

            # 3. Поиск оптимальных назначений
            assignments = self._find_optimal_assignments(
    ...
    def _extract_leftovers_features(
    ...
    def _find_optimal_assignments(
        self, pieces: List[Dict], leftovers_features: List[Dict]
        ...
        for piece in sorted_pieces:
            ...
            for leftover in leftovers_features:
                ...
                piece_tensor = (
                    torch.FloatTensor(piece["features"])
                    .unsqueeze(0)
                    .to(self.device)
                )
                leftover_tensor = (
                    torch.FloatTensor(leftover["features"])
                    .unsqueeze(0)
                    .to(self.device)
                )
                ...
                if (
                    ...
                    and compatibility > self.min_compatibility_threshold
                ):
    ...
    def _predict_new_leftovers(
        self, dxf_data: Dict, assignments: List[LeftoverAssignment]
    ) -> List[Dict[str, Any]]:
        ...
        for (
        ...
        ) in assignments_by_leftover.items():
            ...
            if leftover_info:
                remaining_area = max(
                    0, leftover_info["area"] - total_assigned_area
                )
                ...
                if remaining_area > 5000:  # Минимальная площадь для сохранения
                    new_leftover = {
                        ...
                        "reuse_priority": self._calculate_reuse_priority(
                            remaining_area
                        ),
                        "creation_date": datetime.now().isoformat(),
                        "material_code": leftover_info.get(
                            "material_code", "MDF"
                        ),
    ...
    def _calculate_efficiency_metrics(
        ...
        try:
            ...
            total_piece_area = sum(
                self._get_piece_area_by_id(p["id"], dxf_data)
            ...
            leftover_area_covered = sum(
                self._get_piece_area_by_id(assignment.piece_id, dxf_data)
                * assignment.area_utilization
                for assignment in assignments
            )

            # Потенциальная экономия
            potential_savings = (
                leftover_area_covered / total_piece_area
            ...
            avg_leftover_reuse = (
                np.mean([l.get("reuse_priority", 0) for l in new_leftovers])
                if new_leftovers
                else 0
            )
            ...
            assignment_efficiency = (
                np.mean([a.area_utilization for a in assignments])
                if assignments
                else 0
            )
            coverage_ratio = (
                len(assignments) / len(self._extract_pieces_features(dxf_data))
                if dxf_data.get("entities")
                else 0
            )
    ...
    def _generate_optimized_layout(
        self, assignments: List[LeftoverAssignment], dxf_data: Dict
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for assignment in assignments:
                piece_info = self._get_piece_info(
                    assignment.piece_id, dxf_data
                )
                ...
                if piece_info and leftover_info:
                    ...
                    layout["total_area_utilized"] += (
                        piece_info.get("area", 0) * assignment.area_utilization
                    )
    ...
    def _determine_strategy(self, efficiency_metrics: Dict[str, float]) -> str:
        ...
        leftover_utilization = efficiency_metrics.get(
            "leftover_utilization_rate", 0
        )
    ...
    def _create_leftover_feature_vector(
        ...
        return [
            ...
            leftover.usage_count
            / 10,  # нормализованное количество использований
            leftover.priority,
