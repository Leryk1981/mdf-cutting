...
class CorrectionPredictionModel(nn.Module):
    ...
    def __init__(
        ...
        self.fusion_encoder = nn.Sequential(
            nn.Linear(
                hidden_dims[1] // 2 * 3 + hidden_dims[1] // 4, hidden_dims[1]
            ),
            nn.ReLU(),
            nn.Dropout(0.3),
        ...
        self.correction_params_head = nn.Sequential(
            ...
            nn.Linear(
                hidden_dims[2] // 2, 5
            ),  # dx, dy, rotation, scale_x, scale_y
        )
    ...
    def forward(
        ...
        spatial_topo = torch.cat(
            [features["spatial"], features["topological"]], dim=1
        )
...
class EnsembleCorrectionModel(nn.Module):
    ...
    def __init__(self, feature_dims: Dict[str, int], num_models: int = 3):
        ...
        self.meta_learner = nn.Sequential(
            nn.Linear(
                num_models * 10, 64
            ),  # 10 = 4 (type) + 5 (params) + 1 (improvement)
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(64, 32),
            nn.ReLU(),
...
class HybridCorrectionModel(nn.Module):
    ...
    def forward(
        self, features: Dict[str, torch.Tensor], rl_state: torch.Tensor = None
    ):
        ...
        if rl_state is not None:
            ...
            rl_features = torch.zeros(batch_size, 2).to(
                next(iter(features.values())).device
            )

        # Объединяем признаки
        combined = torch.cat(
            [prediction_output["correction_params"], rl_features], dim=-1
        )

        # Финальные предсказания
        final_params = self.fusion_network(combined)
        confidence = self.confidence_head(
            combined[:, : final_params.size(1) // 2]
        )
...
class AttentionCorrectionModel(nn.Module):
    ...
    def __init__(
        self, feature_dim: int, num_heads: int = 8, num_layers: int = 3
    ):
        super().__init__()
...
class TransformerCorrectionModel(nn.Module):
    ...
    def forward(
        ...
        pos_enc = (
            self.pos_encoding[:seq_len].unsqueeze(0).expand(batch_size, -1, -1)
        )
        x = x + pos_enc
