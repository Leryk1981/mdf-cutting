...
class PackingAlgorithm:
    """
    Алгоритм упаковки деталей на листах материала.

    Использует библиотеку rectpack для оптимального размещения
    прямоугольных деталей на листах материала.
    """

    def __init__(self):
        """Инициализация алгоритма упаковки."""
        self.packer = None

    def pack_details(
        self, details: List[Dict], sheet_width: float, sheet_height: float
    ) -> Dict:
        """
        Упаковывает детали на лист материала.

        Args:
            details: Список деталей для упаковки
            sheet_width: Ширина листа
            sheet_height: Высота листа

        Returns:
            Dict: Результаты упаковки
        """
        # TODO: Реализовать алгоритм упаковки
        return {"status": "packed", "details": details}
...
def hybrid_sort(rectangles):
    ...
    return (
        sorted(large, key=lambda x: -(x[0] * x[1]))
...
def pack_and_generate_dxf(
    ...
    for _, row in unique_combinations.iterrows():
        ...
        for idx, detail in material_details.iterrows():
            ...
            logger.info(
                f"Обработка детали ID={detail['part_id']}, "
            ...
            for q in range(quantity):
                ...
                logger.info(
                    f"Добавлена копия {q+1}/{quantity} детали {detail['part_id']} "
        ...
        for remnant in remnants:
            ...
            packed_ids = (
                set()
            )  # Множество для хранения уже упакованных деталей
        ...
        if remaining_rects and full_sheets:
            ...
            for i, sheet in enumerate(full_sheets):
                ...
                # Добавляем контейнер с размерами за вычетом отступов
                packer.add_bin(
                    sheet["length_with_margin"], sheet["width_with_margin"]
                )
        ...
        for container_type, container_id, packer in all_packers:
            ...
            try:
                ...
                for rect in packer[0]:
                    ...
                    logger.info(
                        f"Добавлена деталь {detail['part_id']} (копия ID: {copy_id}) "
                ...
                if is_remnant:
                    ...
                    if material != "S":
                        ...
                        output_file = (
                            f"sheet_{thickness_int}mm_{sheet_idx}.dxf"
                        )
                    logger.info(
                ...
                organized_file_path = get_organized_file_path(
                    thickness, material, output_file
                )

                # Сохраняем файл в организованную структуру папок
                doc.saveas(organized_file_path)
                logger.info(
                    f"Сохранен файл в организованную структуру: {organized_file_path}"
                )
                layout_count += 1

                # Добавляем контейнер в финальный упаковщик
                final_packer.add_bin(
                    length_with_margin, width_with_margin, bid=bin_counter
                )
    ...
    remnants_count = sum(
        1
        for _, row in current_materials_df.iterrows()
