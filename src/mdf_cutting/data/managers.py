...
def create_material_folder_structure(thickness, material, base_output_dir=""):
    ...
    try:
        ...
        thickness_int = int(thickness) if float(thickness).is_integer() else thickness
        ...
        logger.error(f"Ошибка при создании структуры папок для {thickness}mm_{material}: {str(e)}")
...
def get_organized_file_path(thickness, material, filename, base_output_dir=""):
    ...
    """
    try:
        main_folder_path, _ = create_material_folder_structure(thickness, material, base_output_dir)
