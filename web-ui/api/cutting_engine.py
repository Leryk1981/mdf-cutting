...
try:
    ...
    DEFAULT_KERF = 3.0
    DEFAULT_MARGIN = 5.0
    DETAILS_REQUIRED_COLUMNS = ['part_id', 'length_mm', 'width_mm', 'thickness_mm', 'material', 'quantity']
    MATERIALS_REQUIRED_COLUMNS = ['sheet_id', 'length_mm', 'width_mm', 'thickness_mm', 'material', 'quantity']
...
class CuttingEngine:
    ...
    def validate_details_data(self, details_data: List[Dict]) -> pd.DataFrame:
        ...
        try:
            ...
            missing_cols = [col for col in DETAILS_REQUIRED_COLUMNS if col not in df.columns]
            if missing_cols:
    ...
    def validate_materials_data(self, materials_data: List[Dict]) -> pd.DataFrame:
        """
        ...
        try:
            ...
            # Проверяем обязательные колонки
            missing_cols = [col for col in MATERIALS_REQUIRED_COLUMNS if col not in df.columns]
            if missing_cols:
                raise ValueError(f"Отсутствуют обязательные колонки: {missing_cols}")
    ...
    def optimize_cutting(
        ...
        try:
            ...
            total_used_sheets = max(1, int(len(details_df) / 5))  # Примерный расчет
            ...
            results = {
                ...
                "efficiency": self._calculate_efficiency(details_df, materials_df, total_used_sheets),
    ...
    def _calculate_efficiency(
        ...
        try:
            ...
            for _, detail in details_df.iterrows():
                area = detail['length_mm'] * detail['width_mm'] * detail['quantity']
                total_detail_area += area
            
            # Общая площадь использованных листов
            total_sheet_area = 0
            for _, material in materials_df.iterrows():
                area = material['length_mm'] * material['width_mm'] * material['quantity']
